...
- **Don't expect a green Release run from a non-release push right after publishing.** With no changeset files, `changeset publish` re-checks `npm info`, which can read a **stale CDN 404** for a just-published packument (local edge clears in ~2 min; CI runner edges cache the 404 longer), then tries to **re-publish the already-live version**. `@drizzle-tx`'s pnpm publish then returns `E403 "cannot publish over the previously published version"`, and `@changesets/cli@2.31.0`'s `isAlreadyPublishedError` **crashes** classifying it (`TypeError: … reading 'includes'` — pnpm puts the message in `json.error.message`, changesets reads `json.error.summary`). It's **cosmetic** (the published version is intact) and self-heals as the CDN 404 expires; **real version bumps are immune** (a genuinely-new version is correctly seen as unpublished). Release by bumping versions, not by re-running publish on an unchanged tree.
