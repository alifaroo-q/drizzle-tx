...
- **The `release` job sets `HUSKY: 0`.** When a push carries a pending changeset, `changesets/action` takes the *version* path and runs `git commit -m "Version Packages"`. That commit fires `.husky/pre-commit` (`mise exec -- pnpm lint:fix`); CI has no `mise` on PATH, so the hook exits **127** and the commit — and the whole release job — dies **before the Version PR is opened** (`The process '/usr/bin/git' failed with exit code 1`). This only surfaces on the *first* release path that actually commits (publish-only pushes never commit, so it hid until #11's changeset). Husky hooks are a local-dev convenience; `HUSKY=0` at the job level makes husky v9's `.husky/_/h` wrapper `exit 0` before running any hook. **Don't remove it**, and note the same trap will bite any future CI step that commits.
